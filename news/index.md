...
- [`gh()`](https://gh.r-lib.org/reference/gh.md) now signals a classed
  `gh_interrupt` interrupt when a paginated call is interrupted
...
- GitHub PAT format validation now issues a warning by default instead
  ...
  or the `GH_VALIDATE_TOKENS` environment to `"off"`, `"warn"` or
  `"error"` to configure this.
...
- [`gh()`](https://gh.r-lib.org/reference/gh.md) no longer returns empty
  results when httr2’s HTTP cache revalidates a stored response with
  `304 Not Modified`.

- [`gh()`](https://gh.r-lib.org/reference/gh.md) no longer errors on a
  `304 Not Modified` response. This makes manual conditional requests
...
- New
  [`fake_github_app()`](https://gh.r-lib.org/reference/fake_github_app.md),
...
- [`gh()`](https://gh.r-lib.org/reference/gh.md) no longer crashes when
  reporting a GitHub API error whose `errors` field is a plain string
  rather than the documented array of objects, as happens on some 422
  responses ([\#229](https://github.com/r-lib/gh/issues/229)).
...
[`gh()`](https://gh.r-lib.org/reference/gh.md), and other functions that
use it, now do not save the request headers in the returned object.
Consequently, if you use the
[`gh_next()`](https://gh.r-lib.org/reference/gh_next.md),
[`gh_prev()`](https://gh.r-lib.org/reference/gh_next.md),
[`gh_first()`](https://gh.r-lib.org/reference/gh_next.md) or
[`gh_last()`](https://gh.r-lib.org/reference/gh_next.md) functions and
passed `.token` and/or `.send_headers` explicitly to the original
[`gh()`](https://gh.r-lib.org/reference/gh.md) (or similar) call, then
you’ll also need to pass the same `.token` and/or `.send_headers` to
[`gh_next()`](https://gh.r-lib.org/reference/gh_next.md),
[`gh_prev()`](https://gh.r-lib.org/reference/gh_next.md),
[`gh_first()`](https://gh.r-lib.org/reference/gh_next.md) or
[`gh_last()`](https://gh.r-lib.org/reference/gh_next.md).

### OTHER CHANGES

- New [`gh_token_exists()`](https://gh.r-lib.org/reference/gh_token.md)
  tells you if a valid GH token has been set.

- [`gh()`](https://gh.r-lib.org/reference/gh.md) now uses a cache
...
- [`gh_token()`](https://gh.r-lib.org/reference/gh_token.md) can now
...
- [`gh_next()`](https://gh.r-lib.org/reference/gh_next.md),
  [`gh_prev()`](https://gh.r-lib.org/reference/gh_next.md),
  [`gh_first()`](https://gh.r-lib.org/reference/gh_next.md) and
  [`gh_last()`](https://gh.r-lib.org/reference/gh_next.md) now work
...
- [`gh()`](https://gh.r-lib.org/reference/gh.md) gains a new `.max_rate`
  parameter that sets the maximum number of requests per second.
...
- [`gh()`](https://gh.r-lib.org/reference/gh.md) gains a new `.max_wait`
  argument which gives the maximum number of minutes to wait if you are
  rate limited ([\#67](https://github.com/r-lib/gh/issues/67)).

- New
  [`gh_rate_limits()`](https://gh.r-lib.org/reference/gh_rate_limit.md)
...
- New `.params` argument in
  [`gh()`](https://gh.r-lib.org/reference/gh.md) to make it easier to
  pass parameters to it programmatically
...
- [`gh_gql()`](https://gh.r-lib.org/reference/gh_gql.md) now passes all
  arguments to [`gh()`](https://gh.r-lib.org/reference/gh.md)
...
- Fixed an error that occurred when calling
  [`gh()`](https://gh.r-lib.org/reference/gh.md) with
  `.progress = FALSE` ([@gadenbuie](https://github.com/gadenbuie),
  [\#115](https://github.com/r-lib/gh/issues/115)).

- [`gh()`](https://gh.r-lib.org/reference/gh.md) accepts named `NA`
...
- [`gh()`](https://gh.r-lib.org/reference/gh.md) now silently drops
...
- [`gh()`](https://gh.r-lib.org/reference/gh.md) now returns better
  values for empty responses, typically empty lists or dictionaries
  ([\#66](https://github.com/r-lib/gh/issues/66)).

- [`gh()`](https://gh.r-lib.org/reference/gh.md) now has an `.accept`
  argument to make it easier to set the `Accept` HTTP header
  ([\#91](https://github.com/r-lib/gh/issues/91)).

- New [`gh_gql()`](https://gh.r-lib.org/reference/gh_gql.md) function to
  make it easier to work with the GitHub GraphQL API.

- gh now supports separate personal access tokens for GitHub Enterprise
  sites. See [`?gh_token`](https://gh.r-lib.org/reference/gh_token.md)
  for details.

- gh now supports storing your GitHub personal access tokens (PAT) in
  the system keyring, via the keyring package. See
  [`?gh_token`](https://gh.r-lib.org/reference/gh_token.md) for details.

- [`gh()`](https://gh.r-lib.org/reference/gh.md) can now POST raw data,
  which allows adding assets to releases
