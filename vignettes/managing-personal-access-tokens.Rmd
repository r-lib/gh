...
Please bear with us and feel free to let us know in a GitHub issue.*
...
More resources on how and why to get a PAT:
  ...
  * In the usethis package:
    ...
    - `usethis::create_github_token()` guides you through the process of getting
      a new PAT
  * `gitcreds::gitcreds_set())` helps you explicitly put a PAT into the Git
    credential store.
...
gh now uses the gitcreds package  in the git credential store.
If you use git from the command line, with a GUI or from RStudio, then chances are, you already have a PAT and gitcreds/gh can reuse it.
Call `gitcreds::gitcreds_get()` to try it:

gh calls `gitcreds::gitcreds_get()` with a URL to try to find a matching PAT.
`gitcreds::gitcreds_get()` checks session environment variables and then the local Git credential store. Therefore, if you have previously used a PAT with, e.g., command line Git, gh may retrieve and re-use it. You can call
[gitcreds::gitcreds_get()] directly, yourself, if you want to see what is
found for a specific URL.
